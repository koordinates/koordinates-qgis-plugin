...
class Style:
    ...
    def created_at_date(self) -> Optional[datetime.date]:
        ...
        """
        created_at_date_str: Optional[str] = self.details.get("created_at")
...
class Dataset:
    ...
    def __init__(self, details: Dict):
        ...
        self.geometry_type: QgsWkbTypes.GeometryType = (
            ApiUtils.geometry_type_from_dataset_response(self.details)
        )

        self.capabilities = ApiUtils.capabilities_from_dataset_response(self.details)
        ...
        self.crs: Optional[Crs] = (
            Crs(self.details["data"]["crs"])
            if self.details.get("data", {}).get("crs")
            else None
        )
    ...
    def styles(self) -> List[Style]:
        ...
        results = KoordinatesClient.instance().layer_styles(style_url)
