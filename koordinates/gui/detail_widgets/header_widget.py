...
from qgis.PyQt.QtWidgets import QFrame, QHBoxLayout, QLabel, QSizePolicy, QApplication
...
class HeaderWidget(QFrame):
    ...
    def __init__(self, dataset: Dataset, parent=None):
        ...
        self.publisher_theme = self.publisher.theme if self.publisher else None
        background_color = (
            self.publisher_theme.background_color() if self.publisher_theme else None
        )
        background_color = background_color or QColor("#555657")

        self.setStyleSheet(
            "HeaderWidget {{ background-color: {}; }}".format(background_color.name())
        )
        ...
        if background_color:
            url_frame.setStyleSheet("""QFrame {
                    border-radius: 6px;
        ...
        publisher_site = (
            self.dataset.publisher().site if self.dataset.publisher() else None
        )
