...
from qgis.PyQt.QtCore import pyqtSignal, Qt, QSize, QPointF, QRectF
from qgis.PyQt.QtGui import QColor, QImage, QFontMetrics, QPainter, QPen, QBrush
from qgis.PyQt.QtWidgets import QWidget, QStyleOption, QStylePainter, QSizePolicy

from ..gui_utils import GuiUtils
from ..thumbnails import downloadThumbnail, PublisherThumbnailProcessor
...
class FilterBannerWidget(QWidget):
    ...
    def close_button_rect(self):
        return QRectF(self.width() - 16 - self.HORIZONTAL_MARGIN, 0, 16, self.height())
    ...
    def paintEvent(self, event):
        ...
        painter.drawRoundedRect(
            option.rect, self.THUMBNAIL_CORNER_RADIUS, self.THUMBNAIL_CORNER_RADIUS
        )
        ...
        if self._icon and not self._icon.isNull():
            ...
            painter.drawImage(
                QRectF(
                    option.rect.left() + self.MARGIN_LEFT,
...
class PublisherFilterBannerWidget(FilterBannerWidget):
    ...
    def __init__(self, publisher: Publisher, parent: Optional[QWidget] = None):
        ...
        if self.publisher.publisher_type == PublisherType.Publisher:
            ...
            if self.publisher.theme.background_color():
                ...
                self.set_background_color(QColor("#555657"))
        ...
        if self.publisher.publisher_type != PublisherType.Mirror:
            thumbnail_processor = PublisherThumbnailProcessor(
                self.publisher, QSize(self.THUMBNAIL_WIDTH, self.height())
            )
            if self.publisher.theme.logo():
                downloadThumbnail(
                    self.publisher.theme.logo(), self, thumbnail_processor
                )
            else:
    ...
    def _draw_content(self, event):
        ...
        painter.drawText(
            QPointF(
                left_text_edge,
                option.rect.top() + int(metrics.height() * line_heights[0]),
            ),
            self.publisher.name(),
        )
        ...
        if line_heights[1]:
            painter.drawText(
                QPointF(
                    left_text_edge,
                    option.rect.top() + int(metrics.height() * line_heights[1]),
                ),
                "via " + self.publisher.site.name(),
            )

        if self.publisher.publisher_type == PublisherType.Mirror:
            mirror_image = GuiUtils.get_svg_as_image("mirror_white.svg", 50, 16)
