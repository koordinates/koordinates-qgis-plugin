...
from ..core import KartOperationManager, KartOperation
...
class OperationManagerMessageBarBridge(QObject):
    ...
    def __init__(
        self, operations_manager: KartOperationManager, message_bar: QgsMessageBar
    ):
        super().__init__()
        ...
        self._manager.task_progress_changed.connect(self._report_operation_progress)

        self._manager.single_task_canceled.connect(self._clear)

        self._manager.task_completed.connect(self._report_operation_success)
        self._manager.task_failed.connect(self._report_operation_error)
    ...
    def _set_item_state(
        ...
        if self._current_item and not sip.isdeleted(self._current_item):
            ...
            self._current_item.setLevel(level)
            self._current_item.setDuration(0)
            self._bar.pushItem(self._current_item)
        ...
        if not show_details_button:
            if self._view_details_button and not sip.isdeleted(
                self._view_details_button
            ):
                self._view_details_button.deleteLater()
            self._view_details_button = None
        else:
            if not self._view_details_button or sip.isdeleted(
                self._view_details_button
            ):
    ...
    def _report_operation_progress(
        self,
        ...
        """
        self._update_message(operation, message, tasks_in_progress, progress)
    ...
    def _report_operation_error(
        ...
        self._update_message(operation, message, remaining_tasks, remaining_progress)

    def _report_operation_success(
        self,
        operation: KartOperation,
        message: str,
        remaining_tasks: int,
        ...
        if remaining_tasks == 0:
            ...
            if self._failed_count == 1 and self._completed_count == 0:
                ...
                title = self.tr("{} {} datasets.").format(
                    operation.to_past_tense_string(), self._completed_count
                )
                level = Qgis.MessageLevel.Success
            elif self._completed_count == 0:
                title = self.tr("Failed to {} {} datasets.").format(
                    operation.to_verb(), self._failed_count
                )
                level = Qgis.MessageLevel.Critical
                show_details = True
            else:
                ...
                if self._completed_count == 1:
                    title = self.tr("{} 1 dataset. {} failed.").format(
                        operation.to_past_tense_string(), self._failed_count
                    )
                    show_details = True
                else:
            ...
            self._set_item_state(
                title, level, show_cancel_button=False, show_details_button=show_details
            )
        else:
            show_details = False
            if self._failed_count == 0 and self._completed_count == 0:
                if remaining_tasks == 1:
                    ...
                    title = self.tr("{} {} datasets.").format(
                        operation.to_present_tense_string(), remaining_tasks
                    )
                    show_details = True
                ...
                if remaining_tasks == 1:
                    title = self.tr("{} 1 dataset. {} completed.").format(
                        operation.to_present_tense_string(), self._completed_count
                    )
                    show_details = True
                else:
                ...
                if remaining_tasks == 1:
                    title = self.tr("{} 1 dataset. {} failed.").format(
                        operation.to_present_tense_string(), self._failed_count
                    )
                    show_details = True
                else:
                    title = self.tr("{} {} datasets. {} failed.").format(
                ...
                if remaining_tasks == 1:
                    title = self.tr("{} 1 dataset. {} failed. {} completed.").format(
                        operation.to_present_tense_string(),
                        self._failed_count,
                        self._completed_count,
                    )
                    ...
                    title = self.tr("{} {} datasets. {} failed. {} completed.").format(
                        operation.to_present_tense_string(),
                        remaining_tasks,
                        self._failed_count,
                        self._completed_count,
    ...
    def _show_details(self):
        ...
        if self.table:
            self.table.show()
            self.table.raise_()
            self.table.setWindowState(
                self.table.windowState() & ~Qt.WindowState.WindowMinimized
            )
            self.table.activateWindow()
            return

        self.table = TaskDetailsDialog(self._manager, parent=iface.mainWindow())
